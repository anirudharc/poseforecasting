...
class ucf_dataset(Dataset):  
    def __init__(self, dic, rgb_dir, pose_dir, htmap_dir, mode, transform=None):

        self.keys = list(dic.keys())
        self.values= list(dic.values())
        self.rgb_dir = rgb_dir
        self.pose_dir = pose_dir
        self.htmap_dir = htmap_dir
        self.mode =mode
    ...
    def load_ucf_image(self, ht_path, video_name):
        ...
        openpose_npy, occlusion, frame_mask = self.convert(pose_path, self.htmap_dir, video_name)
    ...
    def __getitem__(self, idx):
        ...
        if self.mode=='train':
            ...
            data['image'], data['heatmaps'], data['occlusion'], data['frameId'] = \
                                            self.load_ucf_image(self.htmap_dir, video_name)
            sample = (data, label)
...
class ucf_dataloader():
    ...
    def train(self):
        training_set = ucf_dataset(dic=self.train_video, rgb_dir=self.rgb_path, pose_dir=self.pose_path, htmap_dir=self.htmap_path, mode='train', 
    ...
    def validate(self):
        validation_set = ucf_dataset(dic=self.val_video, rgb_dir=self.rgb_path, pose_dir=self.pose_path, htmap_dir=self.htmap_path, mode='val', 
            transform = transforms.Compose([
    ...
    def test(self):
        test_set = ucf_dataset(dic=self.test_video, rgb_dir=self.rgb_path, pose_dir=self.pose_path, htmap_dir=self.htmap_path, mode='train', 
            transform = transforms.Compose([
                transforms.RandomCrop(224),
                transforms.RandomHorizontalFlip(),
                transforms.ToTensor(),
